...
export default function ClassQuizzesScreen() {
  ...
  function leaveDraft() { if (dirty) setConfirmation("discard"); else { setQuiz(null); void run(() => load()); } }
  ...
  return <ClassGroupShell title={quiz ? quiz.title || "Prepare a quiz" : "Practice quizzes"} eyebrow={classTitle} onBack={quiz ? leaveDraft : undefined}>
    {!quiz ? <>
      <View style={{ gap: space.sm }}>
        ...
        {teacher ? <ProgramButton label="Create a quiz" icon="plus" emphasis="primary" onPress={newDraft} testID="quiz-create" /> : null}
      </View>
      ...
      {teacher && quiz.status === "draft" ? <View style={card}>
        ...
        {importing ? <View style={{ gap: space.sm }}>
          ...
          <TextInput accessibilityLabel="Questions to import" multiline value={source} onChangeText={setSource} maxLength={60_000} placeholder={QUIZ_EXAMPLE} placeholderTextColor={colors.mutedForeground} style={[t.body, input, { minHeight: 200, textAlignVertical: "top" }]} />
      ...
      {showResults ? <View style={{ gap: space.sm }}>
        ...
        {editing ? <>
          <TextInput accessibilityLabel="Question text" value={q.prompt} onChangeText={prompt => updateQuestion({ prompt })} maxLength={2000} multiline style={[t.title3, input, { minHeight: 96, textAlignVertical: "top" }]} placeholder="What would you like your students to practise?" placeholderTextColor={colors.mutedForeground} />
          <View style={{ flexDirection: "row", gap: space.sm, flexWrap: "wrap" }}>{(["choice", "short"] as const).map(kind => <ProgramButton key={kind} label={kind === "choice" ? "Multiple choice" : "Short answer"} icon={q.kind === kind ? "check" : undefined} onPress={() => updateQuestion({ kind, answer: "", options: kind === "choice" ? ["", "", ""] : [] })} />)}</View>
          {q.kind === "choice" ? q.options.map((option, i) => <View key={i} style={{ flexDirection: "row", alignItems: "center", gap: space.sm }}>
            <Pressable accessibilityRole="radio" accessibilityState={{ checked: Boolean(option) && q.answer === option }} aria-checked={Boolean(option) && q.answer === option} accessibilityLabel={`Correct answer ${String.fromCharCode(65 + i)}`} onPress={() => updateQuestion({ answer: option })} style={{ minHeight: 48, minWidth: 48, borderRadius: radius.md, alignItems: "center", justifyContent: "center", backgroundColor: option && q.answer === option ? colors.actionSoft : colors.muted }}><Text style={[t.bodyStrong, { color: colors.primary }]}>{String.fromCharCode(65 + i)} {option && q.answer === option ? "✓" : ""}</Text></Pressable>
            <TextInput accessibilityLabel={`Choice ${String.fromCharCode(65 + i)}`} value={option} maxLength={500} onChangeText={value => updateQuestion({ options: q.options.map((o, j) => j === i ? value : o), answer: q.answer === option ? value : q.answer })} style={[t.body, input, { flex: 1, minWidth: 0 }]} />
          </View>) : <TextInput accessibilityLabel="Correct short answer" value={q.answer} onChangeText={answer => updateQuestion({ answer })} maxLength={1000} placeholder="Exact accepted answer" placeholderTextColor={colors.mutedForeground} style={[t.body, input]} />}
          {q.kind === "choice" ? <View style={{ flexDirection: "row", flexWrap: "wrap", gap: space.sm }}><ProgramButton label="Add choice" disabled={q.options.length >= 6} onPress={() => updateQuestion({ options: [...q.options, ""] })} /><ProgramButton label="Remove last choice" disabled={q.options.length <= 2} onPress={() => updateQuestion({ options: q.options.slice(0, -1), answer: q.answer === q.options.at(-1) ? "" : q.answer })} /></View> : null}
          <View style={{ flexDirection: "row", gap: space.sm, alignItems: "center" }}><Text style={[t.bodyStrong, { color: colors.foreground }]}>Points</Text><TextInput accessibilityLabel="Question points" keyboardType="number-pad" value={String(q.points)} onChangeText={value => updateQuestion({ points: Number(value) })} maxLength={3} style={[t.body, numeric, input, { width: 96 }]} /></View>
          <ProgramButton label={q.confirmed ? "Question and answer confirmed" : "Confirm question, answer and points"} icon={q.confirmed ? "check-circle" : "check"} disabled={!canConfirm || q.confirmed} onPress={() => updateQuestion({ confirmed: true })} testID="quiz-confirm-question" />
