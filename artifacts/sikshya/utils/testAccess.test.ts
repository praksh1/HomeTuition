...
import { TEST_CLASS_LABEL, bookingNotice } from "./testAccess.ts";
...
test("a test booking says so in the title, which is all a locked phone shows", () => {
  const notice = bookingNotice({ topic: "Algebra", studentName: "Sita", amount: 0, test: true });
  assert.match(notice.title, /TEST/);
  assert.doesNotMatch(notice.title, /booked/, "nothing was bought");
  assert.equal(notice.body, `"Algebra" — ${TEST_CLASS_LABEL}.`);
});

test("`test` is a flag, never a guess from a zero amount", () => {
  ...
  const testWithPrice = bookingNotice({ topic: "Algebra", amount: 500, test: true });
...
test("a booking with no student name still reads as a sentence", () => {
  ...
  assert.equal(bookingNotice({ topic: "Algebra", test: true }).title, "A student joined your class — TEST");
});
...
test("the label is the same sentence the rest of the app paints", () => {
  ...
  assert.match(bookingNotice({ topic: "x", test: true }).body, new RegExp(TEST_CLASS_LABEL));
});
