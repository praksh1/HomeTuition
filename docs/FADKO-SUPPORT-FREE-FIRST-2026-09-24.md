...
7. **Operator workspace:** first sectioned case workspace implemented 26 September (Overview / Class records / Timeline / Decision), with report-versus-evidence labels, missing-info checklist, preserved decision drafts and retryable loading. Non-deploying CI passed; see `.agents/worklog/2026-09-26-codex-support-case-workspace.md` for release state. Deeper linked diagnostics, large-record pagination and dedicated evidence review remain open. Actions remain existing audited human controls; this is not an AI verdict.
