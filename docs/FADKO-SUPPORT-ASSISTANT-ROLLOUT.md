...
- Opening Support starts a new question; recent conversations remain visible and account-scoped, and can be reopened deliberately. Closing the panel does not delete history or a human request. The in-chat **Ask about a different topic** action starts a separate thread.
- Web Enter sends the question; the Send button remains available. The quick choices narrow broad questions. A small set of verified navigation questions has in-app directions even before the Help Library has content; refund, payment, eligibility and account-specific decisions are not guessed.
- English, Devanagari Nepali and selected romanised Nepali abusive terms get a calm tone reminder and a human path. Reports of being bullied or quoted abuse receive empathy, not the same warning. Each is labeled separately for operator review; there is no automatic suspension or ban.
- No automatic chain of consumer chat subscriptions or free API keys is configured. Quota exhaustion must end in a clear human path. Future providers need a privacy review and explicit server-side limits before receiving customer messages.
