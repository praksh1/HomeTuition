# Accepted classroom release: deferred owner observations

Owner accepted Preview on 24 September 2026 and requested production deployment. These are follow-ups, not claimed fixes or reasons to hold that accepted release.

- [x] **Student count parity:** common header button shows unique connected students including the viewer, excluding the teacher. Unknown/reconnecting is not fabricated as zero. The generic socket count is deliberately not used.
- [x] **Teacher phone clear-page access:** Board pages includes Clear this page, with confirmation explicitly naming writing, shapes, images and PDF sheets on this page only. Undo restores the shared material, including file pixels.
- [x] **Teacher mobile participants sheet:** nearly full visible height, safe-area/keyboard handling, scrolling permissions, compact filters, search and existing raised-hand priority. Tested with 50 students/10 raised hands and a keyboard-reduced screen, plus the existing 120-student fixture.
- [x] **Student participant directory and private messages:** public connected names/ids only; private compose stays in the classroom, replies/history remain in Messages. Server enforces shared valid enrollment for student peers, suspended-account checks and bilateral blocks. Safety & help starts a support report; moderation/contact details and other private conversations are never exposed.

Deployed to production at **9e2c418**, 24 September 2026, after full gate **35991929478** passed. Release/test evidence is in `.agents/worklog/2026-09-24-codex-classroom-followups.md`. Physical iPhone/Android follow-up remains useful; automated browser dimensions are not device hardware tests. New age/guardian/consent rules and automatic moderation were not invented.

- [x] Separate navigation follow-up implemented: cold refresh of shared Messages, Sessions and Profile resolves to the signed-in role without losing the intended screen/query. Unverified, incomplete and signed-out gates are unchanged. Feature CI 36029838470 passed 56 built-app refresh/gate checks; production evidence is tracked in the queued-learning-release worklog.

Evidence supplied (not reviewed in this release pass):
- `Screen Recording 2026-09-23 235758.mp4`
- `Video_20260924_000415_219.mp4`

Next priority requested by owner: evidence-grounded AI support with legitimate free API fallback, human-only refunds and bans. This is separate from classroom production approval.
