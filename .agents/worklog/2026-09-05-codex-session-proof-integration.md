...
## Production release — 2026-09-05/06

The product owner manually accepted the synthetic preview ticket at
`https://hometuition-preview.praksh-dhakal.workers.dev/ticket/1` and then explicitly instructed
Codex to proceed with the production release.

### Release source and ordering

- Production `main` was confirmed to be an ancestor of the reviewed
  `codex/session-proof-integration` branch. The release was a clean fast-forward to **`446feb7`**
  (`Document verified Daily webhook contract`), not a reimplementation or a selective copy.
- The final release tree was checked before publishing. The temporary preview ticket seeder was
  absent; no synthetic account, ticket, session, payment, or evidence row was shipped to or
  inserted into production.
- `main` was pushed first so Railway could deploy the API. Codex waited until the new authenticated
  `POST /api/sessions/1/quality` route changed from the old `404` to the expected unauthenticated
  `401` before accepting the frontend rollout. This kept the release API-first.
- The existing GitHub production workflow then built and deployed the website to Cloudflare. It
  completed successfully as run **`34009629616`** in **21m17s**. Cloudflare reported version
  **`89039e2b-a1a0-4cf3-b583-b661228f8126`**.

### Verification performed

Before the push, Codex reran API, Sikshya app, and scripts typechecks; API unit tests; Sikshya unit
tests; the design ratchet; and `git diff --check`. All passed. The design baseline remained
**204 hex literals / 418 raw font sizes**, with no new leaks.

The production workflow passed all 49 configured stages, including compilation, unit rules,
database-backed integration journeys, browser journeys, whiteboard desktop and phone behavior,
phone-photo transfer, slow-phone coverage, in-call chat, failed-call recovery, production export,
Cloudflare deployment, and served-build verification. The step labelled “Deploy is not set up
yet” was correctly skipped because production deployment credentials are configured.

Codex then independently confirmed:

- `https://hometuition.praksh-dhakal.workers.dev/welcome` → HTTP **200**;
- `https://workspaceapi-server-production-5a63.up.railway.app/api/healthz` → HTTP **200** with
  `{"status":"ok"}`;
- the new quality-evidence route → HTTP **401** without credentials, proving the new route is live
  while making no authenticated request and changing no data.

### Deliberately not done

- No purchase, paid-plan activation, payment-gateway transaction, or production test booking.
- No `db:push`, destructive migration, manual production SQL, or synthetic production fixture.
- No Daily webhook secret, recording, or paid Daily capability was enabled. Provider ingestion
  remains safely disabled until the owner makes the documented Daily contract/cost decision.
- No retention sweep was run against production and no automated production retention schedule was
  enabled.
- No post-release documentation commit was pushed to `main`, because that would trigger the entire
  production pipeline a second time for a prose-only change. This release record is kept on a
  documentation-only branch instead.

### Outcome and remaining manual reality

The production API and web app are live at the approved commit. The operator evidence UI can now
summarize evidence attached to genuine production sessions/tickets, but Codex did not manufacture a
production dispute just to populate it. Daily provider evidence will honestly appear unavailable
until Daily ingestion is activated. Real-device behavior for this feature still depends on evidence
being generated by real sessions; the owner should continue ordinary teacher/student testing on the
main website and report any observed discrepancy with the relevant session/ticket number.
